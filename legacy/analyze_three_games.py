#!/usr/bin/env python3
"""
3試合のYouTube動画から自動でシュートチャートとPlay-by-Playを生成

使い方:
    python analyze_three_games.py
"""
...
class ThreeGameAnalyzer:
    def __init__(self):
        ...
        # シュート検出パラメータ
        self.shot_threshold = 50  # ゴールとの距離閾値（ピクセル）
        self.time_window = 2.0    # シュート判定の時間窓（秒）

    def analyze_game(self, video_path: str, game_name: str):
        """1試合を分析"""
        print(f"\n{'='*80}")
        ...
        # ボール検出
        shots = self.detect_shots(cap, fps, total_frames)
        cap.release()
    ...
    def detect_shots(self, cap, fps, total_frames):
        """ボールを検出してシュートを判定"""
        shots = []
        ball_positions = []  # (frame, x, y, confidence)

        # フレームサンプリング（2フレームに1回）
        sample_interval = 2
        sample_frames = list(range(0, total_frames, sample_interval))

        print(f"🔍 ボール検出中... ({len(sample_frames)}フレーム)")
        ...
        for frame_idx in tqdm(sample_frames, desc="検出中"):
            ...
            # YOLO検出
            results = self.model(frame, classes=[32], verbose=False)  # 32 = sports ball
    ...
    def run_analysis(self):
        ...
        for video_path, game_name in games:
            ...
            # 試合分析
            game_data = self.analyze_game(video_path, game_name)
            if game_data:
