# Basketball AI パイプライン 技術解説（旧パイプライン）

> **注記:** 本ドキュメントはRoboflow YOLOv8 + ByteTrackベースの初期パイプラインの技術詳細（ホモグラフィー計算・カメラ視点推定のロジック）を記録したもの。現行パイプラインはSAM3ベースに置き換わっている（[`STATUS.md`](../STATUS.md)、[`docs/CASE_STUDIES.md`](CASE_STUDIES.md)を参照）。ホモグラフィー変換・視点推定の設計自体は現行パイプラインでも考え方を流用しているため、技術記録として残している。
